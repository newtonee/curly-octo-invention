...
//------------------------------------------------------------------------------
//------------------------------------------------------------------------------
double** Legendre_derivatives_matrix(int number_of_harmonics, double phi) //потом сверить с матлабом, т.к. формула может быть неверной
{
	int NOHplus1 = number_of_harmonics + 1;
...
int EGM_reader(string path, int n_max, double** delta_C, double** F)
{
	...
	//что значит c_str() ? - преобразование str в char
	...
	if (file.is_open())//Если открытие файла прошло успешно
	{
		...
		while (getline(file, line))
		{
			...
			//В нашем файле идут два int, а потом 4 числа double
			int n, m;
